...
- **回答可控**：标题栏那颗 **中 / EN** 切换回答语言；**👁** 在纯文本大模型与视觉模型（可**截图提问**）之间切换。
...
- **导入看得见进度、失败有原因**：设置 → 知识库 有 **导入文件…**（可多选）与 **导入文件夹…** 两颗按钮，导入过程按文件实时上报（`导入中 12/37…`），每个文件一行结果：已入库 / 未变化 / 已跳过 / 失败，并写明原因（格式不支持、抽不出文字=扫描版或图片型 PDF、解析失败、索引未就绪未写入、文件超过 50 MB 未读取）。**重复导入内容没变的文件会被跳过**（先比 mtime+大小，再比正文哈希），不再是全量重建。
...
- **隐身模式**：标题栏 **🕶** 让窗口对录屏 / 屏幕共享 / 截图不可见（Windows 有效，macOS 尽力而为），窗口默认也不出现在任务栏——隐藏后用快捷键（默认 `Control+B`）或系统托盘找回。
...
第一次打开应用，看着标题栏一排符号不知道按哪个很正常。它们其实分三组：**左组管采集**（要不要开始听）、
**中组管回答**（答什么、答多长、答给谁看）、**右组是入口**（知识库、设置，其余功能都收在 `⋯` 里）。

| 看到的 | 做什么 | 什么时候用 |
|---|---|---|
| **▶ 开始** / **● 停止** | 采集 / 停止电脑正在播放的系统声音（不是麦克风） | 想开始或结束出字幕，只有这一颗 |
| 输入设备下拉（改用输入采集或 macOS 时才出现） | 选「对方声音」从哪个设备进来 | macOS 要选 BlackHole 等虚拟设备，否则听不到系统声音 |
| **单屏 ｜ 双屏·N** | 答案显示在这台电脑的悬浮窗，还是局域网里的手机；`·N` 是当前在线手机数 | 想把手机当第二屏、电脑上不留窗口 → 双屏 |
| **⚡** | 持续答：像提问的句子自动出一条回答 | 不想每句手点，就点亮它 |
| **👁** | 回答用**纯文本**大模型还是**视觉**模型（可截图提问） | 要把屏幕上的东西给 AI 看时切到视觉 |
| **中 / EN** | AI 回答用中文还是英文 | 面试语言切换，点一下即可 |
| **🎤** | 另开一路，单独转写你自己的发言 | 需要双方都留记录时；建议戴耳机，避免扬声器回声被二次采集 |
| **🕶** | 隐身：窗口对录屏 / 共享 / 截图不可见 | 共享整屏或录制时不想拍到提词窗 |
| **🎚** | 回答风格三组：内容量 × 专业度 × 应答人设 | 会中嫌答得太长、太书面、不像你——改完下一条就生效 |
| **◐** | 面板透明度 30%–100%（只淡背景，文字与图标不变淡） | 提词窗挡住背后的资料时拖一下 |
| **📚** | 直接打开知识库面板（等同 设置 → 知识库） | 批量导入资料、查看逐文件导入结果 |
| **⚙** | 设置中心 | 服务商与 Key、语音识别、手机显示、高级提示词 |
| **⋯** | 溢出菜单：**延迟 HUD** 开关 · **做题** · **诊断信息** · **帮助与教程** · **重新运行配置向导** | 找不常用的功能，先来这颗 |
| **—** | 隐藏窗口（本来也不进任务栏） | 按 `Control+B` 或点托盘图标找回 |
| **✕** | 退出应用 | — |

标题栏之外只有两块：**左侧转录导轨**（对方说了什么）与**右侧提词卡**（你可以怎么答）。导轨里每个泡泡
**悬停**（或 Tab 聚焦）时才浮出 **⧉ 复制 / 译 / ⚡答**，右上角 **⤢** 才展开整屏转录面板，右缘那根细条
拖拽可调宽（10%–50%，重启保留）；提词卡上是 **📄简历 / 📋JD / 🗑** 与一行可折叠的 **历史回答 · N**。

## 🚀 照这个顺序点：四场典型用法

看完功能清单还是不知道从哪下手的话，对号入座即可——每一步都用上面速查表里的按钮。

**A. 只要实时字幕（开会 / 看视频 / 听播客）**

1. 让电脑播放有人说话的内容 → 点 **▶ 开始**。
2. 导轨开始逐句出泡泡；嫌窄就拖右缘细条，想看全貌点 **⤢**。
3. 完事点 **● 停止**。除了一次 Key 配置，这条路不需要任何设置（默认本地 FunASR，音频不出本机）。

**B. 面试 / 洽谈，要 AI 帮你答**

1. 提词卡 **📄简历**、**📋JD** 导入资料（一份 PDF 也可直接 **📚 → 导入文件…**），解析与索引全在本机完成。
2. （可选）点 **🎚** 定内容量与应答人设——怕念不完就选「精简」。
3. 点 **▶ 开始**，然后在对方那句泡泡上点 **⚡答**；不想手点就点亮 **⚡** 持续答。
4. 回答在提词卡流式出现，照着念；点 **历史回答 · N** 回看旧的，**回到最新** 继续跟流。
5. 不想把提词窗拍进共享里 → 点 **🕶**；挡住背后资料 → 拖 **◐**。
6. 换下一场面试：托盘「新建会话」（资料与答案库按会话隔离），**🗑** 一次删干净。

**C. 在线测评 / 笔试（行测、代码题、性格测评）**

1. 点 **⋯ → 做题**，打开小窗（它强制对录屏不可见，没有开关可关）。
2. 先选**题型**，用 **绑定目录** 指给它一个本机题库目录（四个题型各绑各的，也可以都指向同一个总目录）。
3. 在屏幕上**拖框**选中题目 → 读题 → **先查本机题库**（命中约 1ms、完全离线），没命中才交给 AI，AI 不确定时再按需联网。
4. 不想拖框：`Ctrl+Shift+S` 整屏截屏答题 · `Ctrl+Alt+A` 答最新一句 · `Ctrl+Alt+S` 框选截屏。
5. 没有视觉模型也没装本地 OCR 时，用小窗里的 **作答这题** 把题干粘进去，走的是同一条链路。

**D. 把手机当第二屏（电脑屏幕上什么都不显示）**

1. 手机与电脑连**同一个 Wi-Fi**（手机别开移动数据兜底）。
2. 点标题栏 **双屏** —— 连接窗弹出：大二维码 + 只在电脑上显示的 **6 位配对码**。
3. 手机相机扫码（手动输地址也行）→ 手机上请求配对 → 回电脑看那 6 位码输进去。
4. 手机上点一次 **常亮**；连上后连接窗自动收起、主窗隐藏并强制隐身。
5. 点 **▶ 开始**，转写与大字答案开始推送到手机。手机页面顶部的 **控制** 栏可以在手机上开关转录与持续答、改内容量 / 专业度、输入问题、回看本场历史回答。
6. 想完全不回头看电脑截屏答题：连接窗里勾上 **截屏热键只送手机**，之后按 `Ctrl+Shift+S` 即可（拍屏 → 读题 → 查本机题库 → 答案直出手机）。
...
7. 想把自己的发言也单独转写：点标题栏 **🎤**（建议戴耳机，避免扬声器回声被二次采集）。

### 2. AI 回答：⚡答 / 持续答 / 回答语言 / 双通道

- **手动答**：把鼠标移到（或 Tab 聚焦到）导轨里对方那个泡泡上，点浮出的 **⚡答**，提词卡流式生成一段第一人称、可直接照着念的回答；整屏转录面板（⤢）里也照样能逐句作答。
- **持续答**：点亮标题栏 **⚡**，AI 自动接话。门控是三层：明显的寒暄与流程安排（「把这个链接发我一下」）由本地启发式免费挡掉；拿不准的一句才花一次极小的分类调用判断；**判定超时或失败一律回退成「照答」**，宁可多答不漏答。
...
- **回答语言**：点标题栏那颗 **中 / EN** 切换（悬停提示「AI 回答语言」）。此外，在任意一句上选择「翻译」可把该句转录内联翻译成另一语言。
- **文本 / 视觉双通道**：点标题栏 **👁** 切换回答用的模型（悬停提示「纯文本 ⇄ 多模态」）。多模态下可以**截图提问**（把屏幕上的内容作为图片发给视觉模型）。
...
2. **批量资料走知识库面板**：点标题栏 **📚**（等同 设置 → 知识库）——**导入文件…**（一次可多选，一份 PDF 不必为它建目录）与 **导入文件夹…**（递归，跳过隐藏文件与不支持的扩展名）。只接受 `.md/.markdown/.txt/.docx/.pdf/.pptx`；面板下方常驻提示「旧版 .doc / .ppt 请先转成 .docx / .pptx」——那类 97-2003 老格式没有可靠的纯 JS 解析器，选了也会被记成 `格式不支持` 跳过，而不是把乱码写进索引。
3. **导入过程看得见**：按文件实时上报进度（`导入中 12/37…`），每个文件一行结果——**已入库 / 未变化 / 已跳过 / 失败**，原因写明白：`格式不支持`、`抽不出文字（扫描版/图片型）`、`解析失败`（悬停看原始错误）、`索引未就绪，未写入`、`文件超过 50 MB，未读取`（超过上限的文件只按 stat 判断，一个字节都不读）。**重复导入内容没变的文件直接跳过**（先比 mtime+大小，再比正文哈希），不解析、不重新嵌入、不动索引。
...
- **隐身**（标题栏 **🕶**）：窗口对录屏 / 屏幕共享 / 截图不可见（Windows 有效，macOS 尽力而为）。
...
状态栏右侧常驻显示：**「话落→出字」端到端延迟**（末条 / p50 / p95）、**首字延迟**（LLM 第一个字）、**推理耗时**。鼠标悬停每一项都有解释。用途：判断慢在防抖门控、检索还是模型本身——p50 高是普遍慢，末条飙高是偶发卡顿。不想一直看着：标题栏 **⋯ → 延迟 HUD** 关掉。
...
- **设置 → 通用 → 高级设置 → 提示词进阶**（这颗按钮直接打开编辑面板，不必逐层翻）：直接编辑系统提示词的三层——**基础人设模板** / **回答风格指令** / **自定义追加指令**。规则很简单：**输入框留空 = 用内置原文**，框里的灰字就是内置内容本身；每层可单独「恢复默认」，也可「全部恢复默认」。下面的**最终拼接预览**（只读）显示这一次真正发给模型的前缀全文与字数——注意它取当前会话的简历/JD/笔记，所以换一场会话字数就会变。
...
> 这两张是 **v1.0.0 的截图**，本版本不再重拍（截图与核心功能无关，占体积）。v1.0.1 在这之上改了
> 转录栏形态（聊天气泡）、历史回答（默认折叠），并新增标题栏 **🎚** 与 **◐**。**界面以应用实际显示为准**——
> 逐颗按钮是什么意思，看上文 **「🧭 界面速查」**那张表。
...
4. 在对方那句上点 **⚡答** 生成回答；点亮 **⚡** 让它自动接话；在提词卡导入 **📄简历 / 📋JD** 让回答贴合你的经历。

> 想在真实场景里走一遍，看上文 **「🚀 照这个顺序点：四场典型用法」**——只要字幕 / 面试要 AI 答 /
> 在线测评做题 / 手机当第二屏，每条四到六步。
...
## ⬆️ 从 v1.0.0 升级：三处默认行为变了

覆盖安装即可，设置与会话数据不会被删除（两者共用 `%APPDATA%\MeetingAssistant\`）。但有三处默认值
和上一版不同，升级后觉得"哪里不对"通常是它们：

1. **回答默认不再请求「思考」**。v1.0.0 把开关留给服务商，于是支持思考模式的模型常常在你没要求时
   开着思考——首字延迟和 token 都在为你看不见的推理付费。现在默认**关闭**，老 `settings.json` 里没这个
   字段，读取时按「关闭」补齐。想要就在 设置 → 模型 → **思考强度** 选 低 / 中 / 高（该下拉框只对支持
   思考模式的模型出现），选「跟随默认」则一个思考参数都不发。
2. **重复导入内容没变的文件会被跳过**（先比 mtime + 大小，再比正文哈希），结果里显示「未变化」，
   不解析、不重新嵌入、不动索引。再点一次「导入文件…」不再是全量重建。
3. **手机远程控制默认开启**。手机上出现 **控制** 栏不是你配出来的——它默认就开着，判断依据是：能连上的
   设备刚被你拿在手里、并输入过那颗只在电脑上显示的 6 位配对码。不想开：设置 → 通用 → 手机显示 →
   取消勾选 **允许手机远程控制**；只想开一部分：下面六个逐项开关。
...
| 连续回答 开 / 关 | ⚡ 持续答 | 同一个真开关，不是手机侧的本地状态 |
...
**手机上做不到什么**（这几条是设计边界，不是故障）：

- **改不了 Key、模型、会话，也拿不到你的简历或 JD 文本**——手机能碰的只有上面那六件事。
- **思考强度 / 截屏提问 / 切换会话** 仍只在电脑上：思考强度在 设置 → 模型，整屏截屏要在电脑上按热键
  （可配成「只送手机」），会话切换走托盘「新建会话」。
- **有硬上限**：输入框单条最多 **500 字**，历史一次回看最近 **60 条**，命令最多**每秒 5 条**（连点会提示「操作太快，请稍候」）。
- **授权只在连接那一刻读取一次**：在电脑上取消勾选某项之后，**已经连着**的手机页面仍会显示那个按钮，
  点了会被拒绝并给出文字原因；要立刻让按钮消失，重开一次手机页面（或关掉双屏再开）。
...
**The title bar, left to right** — what each glyph actually is, so the first launch is not a scavenger
hunt. Left group controls capture, middle group controls the answers, right group is entry points:

**▶ Start / ● Stop** system audio · input-device dropdown (macOS or input capture) picks where the other
party comes in · **Single | Dual·N** where answers show (N = phones online now) · **⚡** auto-answer ·
**👁** plain-text ⇄ vision model (screenshot Q&A) · **中 / EN** answer language · **🎤** transcribe your own
voice on a second channel · **🕶** stealth from capture · **🎚** length × register × persona ·
**◐** panel transparency · **📚** knowledge panel · **⚙** settings · **⋯** overflow menu (latency HUD,
**Exam**, diagnostics, help & guides, re-run the wizard) · **—** hide (`Ctrl+B` or the tray brings it back) ·
**✕** quit. Outside the bar there are only two surfaces: the transcript rail (hover or Tab a bubble for
**copy / translate / ⚡Ans**; **⤢** opens the full panel; drag the right edge to resize) and the prompt card
(**📄Resume / 📋JD / 🗑** plus the collapsible **Answer history · N** row).

**Four typical sessions, in order** — *captions only*: play anything → **▶ Start** → **● Stop** when done.
*Interview*: import **📄Resume / 📋JD** (or **📚** for a folder) → optionally pick length and persona in
**🎚** → **▶ Start** → **⚡Ans** on their line, or light up **⚡** and let it answer; **🕶** keeps it out of
the recording. *Online assessment*: **⋯ → Exam** → choose the sub-mode and its bank folder → drag-select
the question (bank first, ~1 ms, offline) → LLM only if the bank misses. *Phone as a second screen*:
same Wi-Fi → **Dual** → scan the QR, type the 6-digit code that **only shows on this PC** → tap **Keep
awake** → **▶ Start**; the phone's **控制** bar then drives transcript, auto-answer, length and register.
...
**Upgrading from v1.0.0**: install over the top — settings and sessions are kept (both live in
`%APPDATA%\MeetingAssistant\`). Three defaults differ from the previous version, and "something feels
off" after an upgrade is usually one of them: **thinking is off by default** (a reasoning-capable model
no longer bills you in first-token latency and tokens for reasoning you never asked for — re-enable it
per preset in Settings → Model → **Thinking effort**); **re-importing an unchanged file is skipped** by
mtime + content hash instead of rebuilding the index, shown as *unchanged*; and **remote control from
the phone ships enabled** — a device that got this far was in your hand and typed the 6-digit code that
only ever displays here (Settings → General → Phone display → *Allow remote control* to switch the whole
bar off, or use the six per-item grants). Everything else v1.0.1 adds changes nothing unless you touch
it: an untouched install sends **the same prompt bytes** to the model that v1.0.0 sent. Full notes:
[RELEASE_NOTES_v1.0.1.md](docs/user/RELEASE_NOTES_v1.0.1.md).
