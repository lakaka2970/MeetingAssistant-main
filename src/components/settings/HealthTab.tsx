/**
 * 服务状态 tab of the settings hub (was its own ServiceHealthPanel overlay).
 *
 * The states come from shared/healthState.ts, so this tab, the status-bar
 * chips and the answer-button gating cannot disagree. Nothing here blocks the
 * app: a failed row is information, and the app keeps running around it. The
 * row's fix action now jumps straight to the 模型 tab (this panel can own the
 * chrome), and 打开配置向导 still calls main directly.
 */
...
export function HealthTab({
  ...
  /** jump the hub to the 模型 tab — the settings live there now */
  onOpenModelTab: () => void;
      ...
      <div className="health-row" key={h.key}>
        <div className="conn-line">
          ...
          <span className="health-actions">
            <button className="btn btn-sm" onClick={onOpenModelTab}>
