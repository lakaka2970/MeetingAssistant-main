...
export class RagIngestor {
  ...
  /** embed in worker-sized batches, in the caller's order, before the store moves */
  ...
  async ingest(opts: IngestOptions): Promise<IngestResult> {
    ...
    // hand back a document that is half indexed or gone entirely — and that
    // includes the pairs, which are purged with the body below.
    const vectors = await this.embedAll(chunks);
    const qaVectors = await this.embedAll(pairs.map((p) => p.question));
    ...
    const qa = this.addQaRecords(pairs, qaVectors, opts);
  ...
  async ingestDocSummaries(req: {
    ...
    const vectors = await this.embedAll(summaries.map((s) => s.text));
  ...
  async ingestQaPairs(
    ...
    return this.addQaRecords(pairs, await this.embedAll(pairs.map((p) => p.question)), scope);
  }

  private addQaRecords(
    pairs: QaPair[],
  ...
  async ingestFacts(sessionId: string, facts: string[]): Promise<IngestResult> {
    const vectors = await this.embedAll(facts);
