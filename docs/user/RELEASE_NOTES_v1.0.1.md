...
- **逐文件进度与失败原因**：导入过程按文件实时上报（`导入中 12/37…`），每个文件一行结果——已入库 / 未变化 / 已跳过 / 失败，并给出原因：**格式不支持**、**抽不出文字**（扫描版或图片型 PDF）、**解析失败**（悬停看原始错误）、**索引未就绪，未写入**、**文件超过 50 MB，未读取**（超过上限的文件只按大小判断，不读入内存）。不再出现"点了导入之后界面像冻住"。
