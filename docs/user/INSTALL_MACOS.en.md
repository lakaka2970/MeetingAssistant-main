...
> **Status: no packaged macOS build yet.** The `v1.0.1` release ships Windows installers only. macOS is supported when run from source, and a signed/notarised `.dmg` is planned for a later release. Until then, follow the "Run from source" section below.
