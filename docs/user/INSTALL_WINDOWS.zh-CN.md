...
[Releases 页面](https://github.com/lakaka2970/MeetingAssistant/releases/latest)提供两个文件：

| 文件 | 类型 | 适合 |
|---|---|---|
| `MeetingAssistant-<版本>-win-x64.exe` | 安装版（NSIS） | 常规使用。装完有开始菜单和桌面快捷方式 |
| `MeetingAssistant-<版本>-win-x64-portable.exe` | 免安装版 | 不想装东西、想放在 U 盘或受限电脑上临时用 |
...
```powershell
Get-FileHash .\MeetingAssistant-<版本>-win-x64.exe -Algorithm SHA256
```
...
- 注意：免安装版**不是完全绿色**——设置、会话、简历等数据仍然写在 `%APPDATA%\MeetingAssistant\`，和安装版共用同一份数据。
- 想要真正的「数据也跟着走」，可以在启动前设置环境变量 `MC_USERDATA` 指向你自己的目录，例如：

  ```powershell
  $env:MC_USERDATA = "D:\MeetingAssistantData"
  .\MeetingAssistant-<版本>-win-x64-portable.exe
  ```
...
| 内容 | 路径 |
|---|---|
| 设置（含加密后的 API Key） | `%APPDATA%\MeetingAssistant\settings.json` |
| 会话（转录 + 问答 + 导入的资料） | `%APPDATA%\MeetingAssistant\sessions.json` |
...
2. `%APPDATA%\MeetingAssistant\` 不会被动到，**设置、Key 和会话全部保留**。
...
- 想彻底清干净，卸载后手动删除 `%APPDATA%\MeetingAssistant\` 目录。删除前请确认里面没有你还需要的会话记录。
